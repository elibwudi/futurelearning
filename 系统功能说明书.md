...
> **更新声明 (v1.1)**: 系统底层数据库已由单文件 SQLite 全面升级至关系型数据库 **PostgreSQL 16**。所有的底层数据交互目前由 **Prisma ORM** 接管。以下展示的 SQL 结构现已统一映射至后端的 `server/prisma/schema.prisma` 模型中。
...
**Q: 如何备份数据？**  
A: 复制`server/database.sqlite`文件和`server/public/uploads`目录。
